...
@Component({
  ...
  template: `
    ...
    } } @if (repeat() > 1) { @for (item of Array(repeat() - 1).fill(0); track $index) {
    <div class="mt-4">
      <!-- Recursively render the same skeleton -->
      <app-skeleton-loader [variant]="variant()" [width]="width()" [height]="height()" [repeat]="1">
      </app-skeleton-loader>
    </div>
    } }
  `,
...
export class SkeletonLoaderComponent {
  variant = input<
    'text' | 'paragraph' | 'card' | 'job-card' | 'table-row' | 'avatar' | 'button' | 'image'
  >('text');
